...
// copying this function to create a popup of the fullscreen of the image so I can reuse it elsewhere, namely on profile pic
void universalShowPicFullscreen(BuildContext context, String asset) async {
  double deviceWidth(BuildContext context) => MediaQuery.of(context).size.width;
  double deviceHeight(BuildContext context) =>
      MediaQuery.of(context).size.height;
  List<double> picDimensions = await universalGetPicSize(asset);
  double picHeight = picDimensions[0];
  double picWidth = picDimensions[1];
  bool widerThanTall = picWidth > picHeight;
  // cannot use buildcontext across async gaps requires this mounted check, which requires the whole widget to be stateful
  // copied that code to reuse elsewhere, but can't use mounted by itself outside of state.
  if (context.mounted) {
    return await showDialog(
      context: context,
      builder: (BuildContext context) => Dialog(
...
Future<List<double>> universalGetPicSize(String asset) async {
  final ByteData bytes = await rootBundle.load(asset);
  var decodedImage = await decodeImageFromList(bytes.buffer.asUint8List());
  return [decodedImage.height.toDouble(), decodedImage.width.toDouble()];
}
